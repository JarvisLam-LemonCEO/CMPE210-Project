...
class NatLeastLoadedLB(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # active flow count per backend
        self.active_flows = {b["ip"]: 0 for b in self.BACKENDS}

        # sticky mapping: (client_ip, client_tcp_src, service_port) -> backend_ip
        self.flow_map = {}
    ...
    def on_packet_in(self, ev):
        ...
        # Only handle VIP traffic on supported ports
        ...
        if backend_ip is None:
            ...
            self.logger.info(
                ...
                client_ip, client_tcp_src, backend_ip, service_port, self.active_flows
            )
        else:
        ...
        # Forward DNAT flow: client -> VIP:service_port ==> client -> backend:service_port
        ...
        # Reverse SNAT flow: backend:service_port -> client ==> VIP:service_port -> client
        ...
        # Forward the first packet immediately with NAT actions
